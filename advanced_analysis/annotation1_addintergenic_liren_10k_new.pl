...
#		print "zengxi\n";
		my $id_name = $a[-1];
#		print "###############$id_name####\t";
		($id, $name) = $id_name=~/ID=(.*);\sname=(.*)/i;     # ID=NM_024980;Name=GPR157
#		print "#$id\t$name\n";
...
#				print "$gene;";
#				$flag1 = 1;
#			}elsif($pos >= $spos-10000 && $pos < $spos){
			if($pos >= $spos-1000 && $pos < $spos){
				my @value2 = map {"$_:Promoter"} @value;
				my $gene = join (";", @value2);
				print "$gene;";
				$flag1 = 1;
...
##		my $down_gene = ($right_offset<$count) ? join ("/", @{$record_gff{$chr}{"$right_offset_value\t$right_offset_rightmate"}}) : $sort_chr_array[$#sort_chr_array];
		...
		print "intergenic;  $up_gene(distance:$up_distance),  $down_gene(distance:$down_distance)";
	}
	print "\n";
}
close BK;
